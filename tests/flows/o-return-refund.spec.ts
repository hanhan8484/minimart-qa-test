 ...
 * DEF-008 refundedAt 為 null、畫面「退款時間」空白
 */
test.describe.serial('O-C03 return to refund', () => {
  ...
  test('apply page + submit → 退貨中／待審核 + 受理通知', async ({ page }) => {
    ...
    await expect(page.getByText(COMPLETED_ORDER_ID)).toBeVisible();
    ...
    await expect(
      page.getByText(`訂單 ${COMPLETED_ORDER_ID} 的退貨申請已送出`).first(),
    ).toBeVisible();
  });
  ...
  test('seller review → 已退款 + 通過／退款通知', async ({ page }) => {
    ...
    await page.goto('/notifications');
    await expect(
      page.getByText(`訂單 ${COMPLETED_ORDER_ID} 的退貨申請已通過`).first(),
    ).toBeVisible();
    await expect(page.getByText(`訂單 ${COMPLETED_ORDER_ID} 已退款`).first()).toBeVisible();
  });
  ...
  test('DEF-008: 應記錄並顯示退款時間', async ({ page }) => {
    test.fail(true, 'DEF-008: refundedAt 為 null，詳情「退款時間」空白（R-7.9）');
    await loginAsDemo(page);
    await openOrderDetail(page, COMPLETED_ORDER_ID);
